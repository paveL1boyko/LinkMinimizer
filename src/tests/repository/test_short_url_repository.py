...
@pytest.mark.asyncio
async def test_create_short_url(short_url_repository_fixture):
    """
    ...
    result = await short_url_repository_fixture.create(data)
    response_from_db = await short_url_repository_fixture.get_by_short_code(
...
@pytest.mark.asyncio
async def test_get_by_short_code(short_url_repository_fixture):
    """
    ...
    """
    data: ShortURLDTO = ShortURLDTOFactory.build()
    await short_url_repository_fixture.create(data)
    await short_url_repository_fixture.create(ShortURLDTOFactory.build())

    result = await short_url_repository_fixture.get_by_short_code(
        short_code=data.short_code
    )
...
@pytest.mark.asyncio
async def test_update_click_count(short_url_repository_fixture):
    """
    ...
    result_after_update = await short_url_repository_fixture.update_click_count(
        data.short_code
    )
