...
@pytest.mark.asyncio
async def test_create_short_url_success(short_url_use_case_fixture):
    """
    Test for successful creation of a short URL.
    """
    url_entity = ShortURLEntityFactory.build()
    result = await short_url_use_case_fixture.create(url_entity)
    assert result == url_entity


@pytest.mark.asyncio
async def test_create_short_url_duplicate_error(short_url_use_case_fixture):
    """
    ...
    with patch.object(
        short_url_use_case_fixture.repo, "create", side_effect=DuplicateEntityError
    ):
        with pytest.raises(DuplicateEntityError):
            await short_url_use_case_fixture.create(url_entity)


@pytest.mark.asyncio
async def test_get_by_short_code_success(short_url_use_case_fixture):
    """
    Test for successful retrieval of a short URL by its code.
    """
    url_entity: ShortURLEntity = ShortURLEntityFactory.build()
    await short_url_use_case_fixture.create(url_entity)
    result = await short_url_use_case_fixture.get_by_short_code(
...
@pytest.mark.asyncio
async def test_get_by_short_code_not_found(short_url_use_case_fixture):
    """
...
@pytest.mark.asyncio
async def test_update_click_count(short_url_use_case_fixture):
    """
    Test for updating the click count for a short URL.
    """
    url_entity: ShortURLEntity = ShortURLEntityFactory.build()
    before_update = await short_url_use_case_fixture.create(url_entity)

    await short_url_use_case_fixture.update_click_count(url_entity.short_code)
    after_update = await short_url_use_case_fixture.get_by_short_code(
        short_code=url_entity.short_code
    )
